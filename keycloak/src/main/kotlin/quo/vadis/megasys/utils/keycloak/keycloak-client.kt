package quo.vadis.megasys.keycloak.client
...
class KeycloakClient(val webClient: WebClient, val realm: String, val client: String) {
  companion object {
    val log = logger()
  }
...
class KeycloakAdminCliClient(val webClient: WebClient) {
  companion object {
    val log = logger()
  }

  fun accessToken(username: String, password: String) : Mono<AccessTokenResponse> {
    return KeycloakClient(webClient, "master", "admin-cli").accessToken(username, password)
  }
